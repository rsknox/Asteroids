...
class Sprite:
    ...
    def draw(self, canvas):
        ...
        #print('self.image: ', self.image, ' nimage: ', nimage)
        dlx = nimage[2] // 2
        dly = nimage[3] // 2
        x = self.pos[0]-dlx
        y = self.pos[1]-dly
...
def missile_fire(posn, angle):
    angle_rad1 = (angle * 3.14159) / 180
    velvector = [math.cos(angle_rad1), -math.sin(angle_rad1)] 
...
while not done:
    ...
    #print('Top of loop: ', prog_time)
    #print('Top of loop prog_time: ', prog_time, ' triggers-cc: ',rotate_cclockwise,
                  #' c: ', rotate_clockwise, ' t: ', my_ship.thrust, ' m: ',
                  #msl_tgr)    
    
    # ALL EVENT PROCESSING SHOULD GO BELOW THIS COMMENT
    for event in pygame.event.get(): # User did something
        #print('prog_time: ', prog_time, 'event: ', event)
        ...
        if event.type == KEYDOWN:
            if event.key == K_LEFT:
                ...
                rotate_clockwise = False
                #print('prog_time: ', prog_time, 'cc: ', rotate_cclockwise)
            if event.key == K_RIGHT:
                rotate_cclockwise = False
                rotate_clockwise = True
                #print('prog_time: ', prog_time, 'c: ', rotate_clockwise)
            #if event.key == K_UP:
                #my_ship.thrust = True
                #print('prog_time: ', prog_time, ' my_ship.thrust: ', my_ship.thrust)
            if event.key == K_q:
            ...
            #print('ship size and angle: ', a_missile.pos, a_missile.angle)
                msl_tgr = True
                #print('prog_time: ', prog_time, 'm: ', msl_tgr)
                #missile_fire(a_missile.pos, a_missile.angle)
        #print('P prog_time: ', prog_time, ' triggers-cc: ',rotate_cclockwise,
                  #' c: ', rotate_clockwise, ' t: ', my_ship.thrust, ' m: ',
                  #msl_tgr)  
    ...
    if msl_tgr:
        a_missile.angle = my_ship.get_angle()
        a_missile.pos = my_ship.get_pos()
        #print('missile posn: ', a_missile.pos)
    ...
    #print('rmv rocks: ', rmv_rock_lst, 'rmv msls: ', rmv_msl_lst)
